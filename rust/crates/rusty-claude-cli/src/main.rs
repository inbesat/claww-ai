        ...
        CliAction::Login { output_format } => {
            match output_format {
                CliOutputFormat::Text => {
                    println!("OAuth login is not supported. Please set OPENAI_API_KEY or XAI_API_KEY environment variable.");
                }
                CliOutputFormat::Json => {
                    println!("{}", serde_json::to_string_pretty(&json!({
                        "kind": "login",
                        "message": "OAuth login is not supported. Please set OPENAI_API_KEY or XAI_API_KEY environment variable."
                    }))?);
                }
            }
        }
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    let xai_key_present = env::var("XAI_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());

    let has_credentials = openai_key_present || xai_key_present;
    ...
    DiagnosticCheck::new(
        ...
        if has_credentials {
            "API credentials configured (OPENAI_API_KEY or XAI_API_KEY)"
        } else {
            "no API credentials found; set OPENAI_API_KEY or XAI_API_KEY"
        },
    )
    .with_details(vec![format!(
        "Environment       openai_key={} xai_key={}",
        if openai_key_present { "present" } else { "absent" },
        if xai_key_present { "present" } else { "absent" },
    )])
    .with_data(Map::from_iter([
        ("openai_api_key_present".to_string(), json!(openai_key_present)),
        ("xai_api_key_present".to_string(), json!(xai_key_present)),
    ]))
}
...
impl ApiRuntimeClient {
    fn new(
        ...
        let client = ProviderClient::from_env(&model)?;
        Ok(Self {
...
fn detect_provider_for_model(model: &str) -> String {
    use api::ProviderKind;
    match api::detect_provider_kind(model) {
        ProviderKind::OpenAi => "OpenAI-compatible".to_string(),
        ProviderKind::Xai => "xAI (Grok)".to_string(),
...
impl ApiClient for ApiRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        eprintln!("[INFO] Using model: {}, provider: {}", self.model, self.provider);
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        let error = ApiError::ContextWindowExceeded {
            ...
            estimated_total_tokens: 182_512,
        ...
        assert!(
            rendered.contains("Total estimate   ~182512 tokens (heuristic)"),
            "{rendered}"
        );
